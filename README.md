...
| async getHasNew() | { hasNewMessages: boolean, hasNewInvites: boolean } | Cheap single round-trip check for any new activity, intended for the cold-start case before subscriptions are live. Subscribed clients should derive freshness from the live event streams instead. |
...
| onCreateMessagesSystemRemoved(handler: function) | messages: Message[] | { oldestMessagesByConversationId: Map<string, Message>, updatedParticipantActivities: ParticipantActivity[] } | Bulk-insert "messagesRemoved" system messages, one per input message. Skip per-conversation if the conversation's current oldest message is already a "messagesRemoved" system message. For every input conversationId, return its resulting oldest message: the just-inserted one or the pre-existing system message that caused the skip. Clamp older participant activities onto it, returning the updated rows as `updatedParticipantActivities`. No participation check and no `lastActivityAt` bump. |
...
| onReadHasNew(handler: function) | participantId: string | { hasNewMessages: boolean, hasNewInvites: boolean } | Should return whether the participant has any unseen invites and any conversation where the latest message is newer than their participant activity's `lastReadMessageCreatedAt`. Implement as one query. |
...
| onDeleteMessagesBefore(handler: function) | thresholdDate: Date | { affectedConversationIds: string[] } | Should delete all messages whose `createdAt` is before `thresholdDate`, plus their reactions. Return the conversationIds that had at least one non-"messagesRemoved" message deleted (deduplicated), and delete expired "messagesRemoved" system messages only in those conversations. Deleting them elsewhere would re-post them daily in dormant conversations. |
...
`"messagesRemoved"` is posted by the daily `messageAfterDays` cleanup as a placeholder so conversations don't look empty or truncated when older messages age out, with older participant activities clamped onto it so it never counts as unread.
